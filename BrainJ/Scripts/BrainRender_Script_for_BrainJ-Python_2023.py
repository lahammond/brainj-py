# brainrender script for plotting cells from BrainJ-Python
...
brainrender.settings.ROOT_ALPHA = 0.05  # transparency of the overall brain model's actor'

#%%

# Provide path to ClearMap cells csv file that you wish to plot
cellsfile1 = "D:/BrainJ Datasets/Processed/A1/5_Analysis_Output/Cell_Analysis/C2_Annotated_Cells.csv"
...
regions = [672, 223] #[672, 223]
acronyms = [] #["CP", "ARH]



#%%
def read_in_BrainJ_Python_cells(filename, regions, acronyms):
    ...
    cells = pd.read_csv(filename, usecols=[0,1,2,14], names = ['X','Y','Z','ID'], #, usecols=col_list)
                          skiprows = [0]) #skip header row
    
    #! If there is an error - likely due to ID column being incorrect - check printed cells, updated to the correct column
    print(cells)
    ...
    #! Modify according to axis order
    Z = cells['X'].tolist()
    X = cells['Z'].tolist()
    Y = cells['Y'].tolist()
    #ID = cells['ID'].tolist()

    #If orientation was flipped during processing then reverse the order of that axis such as:
    #Z = Z[::-1]
...
# use brainJ-python importer function to read in cell coordinates
coordinates1 = read_in_BrainJ_Python_cells(cellsfile1, regions, acronyms)
#plot every 10th coorinate to save time:
#coordinates1 = coordinates1[::10]
...
scene = Scene(title="Cells from BrainJ - ARH and CP")
...
scene.add_brain_region("ARH", alpha=0.35)
